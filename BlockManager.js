...
export class BlockWorker {
    ...
    getRelativeBlock(x, y, z) {
        return this.bot.blockAt(this.bot.entity.position.offset(x, y, z))
    }
    ...
    async placeBlock(targetPos) {
        ...
        if (!placeableBlock) {
            this.bot.chat("Not Enough Blocks!")
            return false
        }
        ...
        for (const dir of directions) {
            const attachBlock = this.bot.blockAt(targetPos.offset(dir.x, dir.y, dir.z))
            if (attachBlock && attachBlock.boundingBox === 'block' && !isFluid(attachBlock)) {
                try {
                    await this.bot.placeBlock(attachBlock, dir.scaled(-1))
                    return true
                } catch (e) {
                    continue
                }
